...
sudo -u postgres echo "host $DB_NAME u_user_api 0.0.0.0/0 md5" >> $PG_HBA_CONF
sudo -u postgres echo "local $DB_NAME u_user_api 0.0.0.0/0 md5" >> $PG_HBA_CONF

echo "Adjusting permissions for u_staff_api..."
sudo -u postgres echo "host $DB_NAME u_staff_api 0.0.0.0/0 md5" >> $PG_HBA_CONF
sudo -u postgres echo "local $DB_NAME u_staff_api 0.0.0.0/0 md5" >> $PG_HBA_CONF

echo "Adjusting permissions for u_admin..."
sudo -u postgres echo "local $DB_NAME u_admin 0.0.0.0/0 md5" >> $PG_HBA_CONF
